...
export async function getMinecraftVersions() {
    ...
    return (await (response.json() as Promise<{ "versions": MinecraftVersion[] }>)).versions.filter(version => version.type === "release");
}

export async function getMinecraftProtocolVersions() {
    // JSON from https://wiki.vg/Protocol_version_numbers for all versions and their protocol numbers
    const response = await fetch("https://raw.githubusercontent.com/skyrising/mc-versions/main/data/protocol/netty.json");
    if (!response.ok) {
        throw new Error(response.statusText);
    }
